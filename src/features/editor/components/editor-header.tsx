...
export const EditorNameInput = ({ workflowId }: { workflowId: string }) => {
  ...
  const [name, setName] = useState(''); // initialize safely
  const inputRef = useRef<HTMLInputElement>(null);

  // Update local state when workflow loads or changes
  useEffect(() => {
    if (workflow?.name) {
  ...
  if (!workflow) return null; // don't render until workflow is loaded
